// Isolated WeSpeaker ResNet34 encoder-forward benchmark.
//
// Times ONLY the encoder graph (FBank computed once, excluded), so the number is
// directly comparable to the Winograd/MLAS @1t latencies. Threads via --threads.
// Routing is selected by the VOICEDETECT_WINOGRAD / VD_CONV2D env vars the
// production conv2d_auto already honours.
#include "encoder.hpp"
...
int main(int argc, char** argv) {
    ...
    vd::ResNet34Encoder enc(ml);
    std::vector<float> pre;
    // warmups (also triggers the one-time weight pack / U build)
    ...
    std::fprintf(stderr, "[bench wespeaker] threads=%d T=%d N=%d  median=%.2f ms  best=%.2f ms  mean=%.2f ms\n",
